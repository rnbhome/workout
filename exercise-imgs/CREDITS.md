# Image credits

Images sourced from [free-exercise-db](https://github.com/yuhonas/free-exercise-db) (MIT license).

Each exercise uses two frames (start + end) animated client-side as a 2-frame loop.

| Local id | Source exercise | Frame 0 | Frame 1 |
|---|---|---|---|
| `pushup-holds` | Plank | `Plank/0.jpg` | `Plank/1.jpg` |
| `thoracic-rotation` | Torso Rotation | `Torso_Rotation/0.jpg` | `Torso_Rotation/1.jpg` |
| `leg-extensions` | Front Leg Raises | `Front_Leg_Raises/0.jpg` | `Front_Leg_Raises/1.jpg` |
| `side-hip-abduction` | Side Leg Raises | `Side_Leg_Raises/0.jpg` | `Side_Leg_Raises/1.jpg` |
| `hip-flexion` | Hip Flexion with Band | `Hip_Flexion_with_Band/0.jpg` | `Hip_Flexion_with_Band/1.jpg` |
| `heel-raises` | Standing Calf Raises | `Standing_Calf_Raises/0.jpg` | `Standing_Calf_Raises/1.jpg` |
| `squats` | Bodyweight Squat | `Bodyweight_Squat/0.jpg` | `Bodyweight_Squat/1.jpg` |
| `lunges` | Bodyweight Walking Lunge | `Bodyweight_Walking_Lunge/0.jpg` | `Bodyweight_Walking_Lunge/1.jpg` |
| `shoulder-flexion` | Front Dumbbell Raise | `Front_Dumbbell_Raise/0.jpg` | `Front_Dumbbell_Raise/1.jpg` |
| `shoulder-abduction-standing` | Side Lateral Raise | `Side_Lateral_Raise/0.jpg` | `Side_Lateral_Raise/1.jpg` |
| `pec-stretch` | Chest And Front Of Shoulder Stretch | `Chest_And_Front_Of_Shoulder_Stretch/0.jpg` | `Chest_And_Front_Of_Shoulder_Stretch/1.jpg` |
| `shoulder-external-rotation` | External Rotation | `External_Rotation/0.jpg` | `External_Rotation/1.jpg` |
| `shoulder-abduction-90` | Seated Side Lateral Raise | `Seated_Side_Lateral_Raise/0.jpg` | `Seated_Side_Lateral_Raise/1.jpg` |
| `bicep-curls` | Concentration Curls | `Concentration_Curls/0.jpg` | `Concentration_Curls/1.jpg` |
| `pronation-supination` | Wrist Rotations with Straight Bar | `Wrist_Rotations_with_Straight_Bar/0.jpg` | `Wrist_Rotations_with_Straight_Bar/1.jpg` |
| `wrist-flex-ext` | Palms-Up Dumbbell Wrist Curl Over A Bench | `Palms-Up_Dumbbell_Wrist_Curl_Over_A_Bench/0.jpg` | `Palms-Up_Dumbbell_Wrist_Curl_Over_A_Bench/1.jpg` |
| `posterior-fly` | Reverse Flyes | `Reverse_Flyes/0.jpg` | `Reverse_Flyes/1.jpg` |
| `shoulder-internal-rotation` | Internal Rotation with Band | `Internal_Rotation_with_Band/0.jpg` | `Internal_Rotation_with_Band/1.jpg` |
